...
**2026-04-27**: A subsequent refactor consolidated the publish-time gating logic in `publish_projects` around two `Project` query methods (`is_releasable_under` and `is_prepared_for_release`). As a result, the changelog guard now applies to all *releasable* projects rather than only to public (registry-publishable) ones. In particular, private packages opted into tag-only release via `[git].publish_private_packages` (per [ADR-043](043-publish-private-packages-to-github-releases.md)) are now subject to the same `CHANGELOG.md` check before they receive git tags and GitHub Releases. Truly private packages -- those not listed in `publish_private_packages` -- continue to be silently excluded prior to the guard, consistent with [ADR-007](007-honor-private-packages-during-publish.md). This supersedes the Decision section's framing of the guard as running "after silently skipping private packages" and applying to "public packages," and the Neutral bullet's claim that "the changelog guard runs after the private-package check and only applies to public packages." The guard's semantics and rationale are otherwise unchanged.
