...
As the project matured, wizard modules grew substantially. The `tui::init` module reached approximately 2000 lines after [ADR-019](019-improved-init-workflow.md) expanded it to eight screens, and `tui::change` reached approximately 950 lines with two screens. Each screen carries its own handler logic, rendering logic, and a full test suite covering both handlers and rendering -- all colocated in a single file. Navigating, reviewing, and testing individual screens became increasingly difficult as unrelated screen code competed for attention in the same file.
