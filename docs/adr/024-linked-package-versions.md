...
### 2026-03-15: Linked-version reconciliation now runs in two passes

The "Version calculation algorithm" section (step 4) and the first bullet of the Neutral consequences both describe linked-version reconciliation as a single step running after changeset aggregation and before version bumping. With dependency propagation introduced by [ADR-023](023-dependency-propagation-bumps.md), the implementation now performs two passes: a first pass (`reconcile_linked_versions`) that runs exactly as described here, plus a second pass (`sync_linked_groups_after_propagation`) that runs after dependency propagation and, if propagation raised a linked-group member beyond the group target, promotes the rest of the group to match by computing each member's effective new version rather than re-deriving a bump level (which would misread the first pass's synthetic entries). The second pass preserves convergence, monotonicity, and single-version-per-group; the original algorithm remains accurate for the first pass and the second is purely additive.
