...
- **2026-05-13**: The Neutral bullet stating "`GitHubRepo` struct [...] only their call site moves from command handlers to `main.rs`" and the bullet stating "The `github/` module directory retains its name per [ADR-041](041-rename-github-client-trait-to-code-forge-client.md)" both become stale with [ADR-056](056-gitlab-support-client-config-and-ci.md): the `github/` module is relocated to `forge::github`, `GitHubRepo` and `GitHubRepo::resolve()` move with it, and resolution is now performed in `cursus-bin/src/forge_resolution/github.rs` (extracted from `main.rs`). The ADR-056 work also adds a parallel `GitLabProject { host, group, project }` identity inside `forge::gitlab`, which is the second concrete instance of the per-client identity model this ADR introduced. The decision of storing repo identity at construction time stands unchanged; only its physical location and the second instance have changed.
