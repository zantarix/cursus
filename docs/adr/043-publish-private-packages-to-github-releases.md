...
### 2026-04-27: Listed private packages now subject to the changelog guard

The Decision section's treatment of `[git].publish_private_packages` packages does not mention the [ADR-031](031-changelog-guard-for-unprepared-packages.md) `CHANGELOG.md` guard, which is no longer accurate. A subsequent refactor of `publish_projects` consolidated release-readiness gating around `Project::is_releasable_under` and `Project::is_prepared_for_release`, so listed private packages are now subject to the same `CHANGELOG.md` guard before they receive git tags and GitHub Releases. They naturally satisfy the guard because `cursus prepare` always creates `CHANGELOG.md`; the change is observable only when the changeset-based flow has been bypassed for such a package, in which case the package is now warned and skipped rather than receiving an unprepared tag/release.
