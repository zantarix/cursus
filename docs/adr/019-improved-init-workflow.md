...
- **2026-05-21:** The `EnableGitHub` Yes/No screen described in the wizard flow (step 6) has been replaced by a three-way `ChooseForge` prompt (GitHub / GitLab / Neither) as part of [ADR-057](057-cursus-init-gitlab-support.md). The "Branch strategy implies GitHub" shortcut from step 5 is preserved by defaulting the new prompt's selection to GitHub when Branch was chosen, but the user can now actively pick GitLab or Neither at that point. The companion `EditGitHub` screen and its auto-detection behaviour are unchanged. See [ADR-057](057-cursus-init-gitlab-support.md) for the new flow and the new `EditGitLab` screen.
- **2026-05-21:** The init config-template writer now emits user-opted-in sections (Cargo, npm, git, and the chosen forge) first as active TOML, with always-commented and disabled sections following as commented-out templates. The canonical section sequence (`[global]`, cargo, npm, `[prepare]`, `[linked-versions]`, `[git]`, `[github]`) is preserved within each group, and the "every option remains discoverable as a commented-out line" principle from this ADR is preserved. See [ADR-057](057-cursus-init-gitlab-support.md) for context.
