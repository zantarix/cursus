...
ADR-001 established the `chronicle init` command with a two-screen TUI wizard that asks only one meaningful question: which package manager to use. Since then, Chronicle has grown to include git lifecycle automation (`[git]` section, ADR-006/ADR-015), GitHub Releases (`[github]` section, ADR-005), and a CI-managed workflow (`chronicle ci`, ADR-015). These features all require configuration in `.chronicle/config.toml`, but `init` provides no guidance for any of them.
...
We will expand `chronicle init` into a purely interactive TUI wizard that guides users through four concerns: package managers, manifest paths, git automation, and GitHub integration. The wizard uses progressive disclosure so that users who want a simple setup answer fewer questions. There will be no non-interactive CLI flags; the existing `--package-manager` flag will be removed. Scripts that need to create a config can write the TOML file directly.
...
1. **ConfirmOverwrite** -- Shown only when `.chronicle/config.toml` already exists. Displays a clear warning that proceeding will overwrite the existing configuration. Yes/No toggle, default No. When no config exists, this screen is skipped entirely and the wizard begins at the PackageManagers screen.
...
Running `chronicle init --no-interactive` will produce an error message explaining that init is interactive-only and suggesting that scripts write the config file directly.
...
# release_branch_prefix = "chronicle-release/"   # Prefix for release branches (branch strategy)
...
- `[git].release_branch_prefix` -- the default `chronicle-release/` is almost always correct
...
- Removing `--package-manager` and non-interactive mode is a breaking change for any existing scripts that call `chronicle init`. However, init is typically run once interactively per repository, and scripts can write the TOML file directly as a straightforward migration.
...
- The ConfirmOverwrite screen replaces the old Confirm screen. The confirm-before-setup prompt is removed for fresh repositories, making init slightly faster for the common case. Existing repositories that already have a config see a clear overwrite warning instead of a generic "Set up Chronicle?" prompt.
