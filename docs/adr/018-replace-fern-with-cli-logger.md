...
The `log` facade decision from ADR-013 is unchanged. All call sites continue to use `log::info!()`, `log::warn!()`, etc. The level filter, stream routing, and formatting behaviour are preserved exactly.
...
- ADR-014's verbose/silent flag mechanism continues to work identically, as it only adjusts the level filter value passed to `log::set_max_level()`

## Alternatives Considered

### env_logger

The most widely used `log` backend. Rejected because its `Builder::target()` method sets the output stream globally for all log levels -- it cannot route `Info` to stdout and `Warn` to stderr. This was the same reason it was rejected in ADR-013, and nothing has changed.
