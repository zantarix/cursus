...
Cursus's release workflow is currently a three-step manual process:

1. **`cursus release`** — Updates the filesystem (version bumps, changelog generation, changeset deletion)
2. **Manual git operations** — User commits changes, creates tags, and pushes to remote
3. **`cursus publish`** — Publishes packages to registries and creates GitHub Releases ([ADR-004](004-publish-command.md), [ADR-005](005-github-releases.md))

[ADR-003](003-release-command.md) explicitly states that Cursus "intentionally does not handle the commit step" because "users run different CI systems, may want different commit strategies, and may require GPG signing or other policies that Cursus should not assume."

Without git automation, there is a manual gap between steps 1 and 3. After running `cursus release`, users must manually:
...
- `run_until`: Controls how far through the git lifecycle Cursus proceeds. Git operations are sequential — each step implies all previous steps:
...
- `extra_files`: Additional file paths (relative to the git root) to stage before committing. Paths are resolved against the git working directory and validated to not escape the repository root. Staging an unmodified file is a no-op in git, so it is safe to list files that may not have changed. This is useful when a custom `lock_command` is configured and Cursus cannot determine which lock file the command writes. Defaults to an empty list.

### CLI override

`cursus release` accepts a `--no-git` flag that disables all git operations for that invocation, regardless of the `[git]` configuration:

```text
cursus release --no-git
```
...
`--no-git` composes with `--dry-run`: `cursus release --dry-run --no-git` skips both filesystem and git operations, and the dry-run output omits the git summary.

### Behaviour

Git hooks execute **after** `cursus release` completes all filesystem modifications (version bumps, changelog generation, changeset deletion) but **before** printing the final summary.

The steps run sequentially, up to and including the step specified by `run_until`:

#### 1. Commit (always runs when git is enabled)

- **What**: Create a git commit with all Cursus-modified files
- **Files staged**: All files modified or deleted by Cursus during the release:
  - Modified manifest files (`Cargo.toml`, `package.json`)
  - Modified or created `CHANGELOG.md` files
  - Deleted changeset files (`.cursus/*.md`)
...
- **Commit message format**:
  ...
  Examples:
  - Single package: `chore(release): cursus-cli@0.2.0`
  - Multiple packages: `chore(release): cursus-cli@0.2.0, @mscharley/cursus@1.0.0`
- **Unstaged files**: Cursus only stages the files it modified. Any other uncommitted changes in the working tree are left untouched.
...
- **Tag message**: `Release {package} version {version}`
  - Example: `Release cursus-cli version 0.2.0`
...
When `cursus release --dry-run` is invoked:
...
**Cursus's error handling policy:**

- Filesystem modifications are NOT rolled back on git failure
- The release has already happened from Cursus's perspective (versions bumped, changelogs written, changesets deleted)
- Git failures are reported clearly with the underlying git error message
- Cursus exits with a non-zero status code
...
[ADR-003](003-release-command.md) (Accepted) states: "Cursus intentionally does not handle the commit step."
...
[ADR-005](005-github-releases.md) (GitHub Releases) creates GitHub Releases during `cursus publish`, identified by git tags. The two ADRs share responsibility for tag naming and creation.
...
- For backward compatibility, if `[github].tag_format` is present but `[git].tag_format` is not, Cursus uses the value from `[github]` and prints a deprecation warning

**Tag creation:**

- When `[git].enabled = true` and `run_until` is `"tag"` or `"push"`, Cursus creates git tags during `cursus release`
- When `[git].enabled = false`, tags must already exist before running `cursus publish`, and the user or CI is responsible for creating them
...
- Improves GitHub Releases workflow: when combined with [ADR-005](005-github-releases.md), users can go from changesets to published packages with GitHub Releases in a single command sequence: `cursus release && cursus publish`.
...
- Couples Cursus to git: Cursus now invokes git commands and must handle git failures. Previously, Cursus was filesystem-only.
...
- Error handling complexity: git operations can fail in many ways (network errors, authentication, conflicts). Cursus must detect and report these clearly without losing the release work.
...
- Error path tests should simulate git failures (no remote, authentication required, merge conflicts) and verify Cursus reports errors without rolling back the release.
