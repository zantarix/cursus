...
### 2026-05-25: npm bundle transport no longer the attestations API

Step 2 of the Decision's verification sequence ("Fetch attestation bundle" via an unauthenticated `GET https://api.github.com/repos/zantarix/cursus/attestations/sha256:<digest>`) is now incorrect, and with it the two Negative Consequences that follow from it: the claim that "the postinstall script now requires two GitHub network endpoints (Releases asset download plus attestations API)" and the entire 60-request-per-hour unauthenticated REST rate-limit hazard no longer describe the npm channel. [ADR-061](061-token-free-cross-platform-artifact-verification.md) moves the npm postinstall onto a Sigstore bundle co-located with the binary as a `<artifact>.sigstore.json` Release asset, fetched over the same un-rate-limited CDN path as the binary and verified offline, so the postinstall makes no GitHub API call and is no longer subject to the 60/hr limit. The trust model, the bundled `sigstore-js` verifier, the pinned SAN, and the OIDC-issuer check are all unchanged — only where the bundle bytes come from changed.
