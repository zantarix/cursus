...
### 2026-05-13: Per-package artifact layout extended to GitLab

[ADR-056](056-gitlab-support-client-config-and-ci.md) adds a parallel `[gitlab.artifacts.<package-name>]` table to the config schema, using the same `BTreeMap<String, BTreeMap<String, String>>` shape and the same no-merge/no-inheritance rule introduced here. To keep callers forge-agnostic, `Config` gains a `forge_artifacts()` helper that returns the active forge's per-package map (GitHub-first when both forges are enabled). The decision of this ADR — per-package keying with explicit display-name-to-path values, no fallthrough between packages — is unchanged; GitLab simply inherits the same model. The GitLab upload path goes through the Generic Package Registry rather than the GitHub Releases asset endpoint, but that is an implementation detail hidden behind `CodeForgeClient::upload_asset` and does not change the configuration surface.
