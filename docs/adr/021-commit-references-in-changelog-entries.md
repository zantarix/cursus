...
### 2026-05-24: Reference extraction is now multi-forge and self-describing

The claims in this ADR that reference extraction is "tied to GitHub's commit message conventions" and "will silently produce no PR number for non-GitHub workflows" are now incorrect, as is the description of reading the commit message via `git log -1 --format=%s` (subject only) and storing a GitHub-specific `pr_number` field. Following the GitLab support work begun in [ADR-056](056-gitlab-support-client-config-and-ci.md), the feature now detects both GitHub pull requests (`#NNN`) and GitLab merge requests (`!NNN`) as self-describing tokens, with no forge configuration consulted by detection or rendering. Detection reads the full commit message body (because GitLab's `See merge request <path>!NN` line lives in the merge commit body) and applies an ordered precedence — GitHub merge, GitLab merge, GitHub squash, GitLab squash — anchoring the squash patterns to the first line to avoid false positives. The stored value is now a `ForgeReference` enum rather than an `Option<u64>` PR number, and rendering emits `via #NNN` for GitHub and `via !NNN+` for GitLab (the trailing `+` makes GitLab inline the MR title, with any cross-project path preserved, e.g. `via group/proj!71+`); an invalid GitLab project path causes the pattern to be treated as unmatched rather than emitting an unsafe link.
