...
### 2026-05-22: Late-guard exception now applies to two forge decorators

The 2026-04-29 erratum above names the only late-guard exception as `SignedCommitGit::commit()` against the GitHub Git Data API. That framing is now incorrect: [ADR-058](058-verified-release-commits-on-gitlab-via-web-commits-api.md) renames the GitHub decorator to `GitHubSignedCommit` and adds a sibling `GitLabSignedCommit` decorator that carves out the same exception against GitLab's commits API. The pattern itself is unchanged — each decorator captures a `dry_run` flag at construction time and short-circuits before any HTTP request — but the exception now exists at two call sites rather than one, and the decorator the older erratum names by its pre-rename name no longer exists under that name.
