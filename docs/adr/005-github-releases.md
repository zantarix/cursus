...
### 2026-03-09: Tag creation moved from release to publish

The "Tag creation" section's claim that "Cursus creates git tags during `cursus release`" (gated by `run_until` being `"tag"` or `"push"`) is no longer accurate. [ADR-015](015-ci-managed-release-workflow.md) replaces the `run_until` field with `strategy` and moves tag creation entirely into `cursus publish`; when `[git].enabled = true`, `publish` itself creates and pushes the tags rather than requiring them to already exist.
...
### 2026-04-06: Flat `[github.artifacts]` map replaced by per-package sections

The flat `[github.artifacts]` configuration shape (`BTreeMap<String, String>` of display-name → path) described in this ADR is no longer the on-disk format. [ADR-044](044-per-package-github-release-artifacts.md) restructures it into per-package sections (`[github.artifacts.<package-name>]`) so that monorepos can scope artifacts to the package being released. The `build_command` behaviour and upload semantics are otherwise unchanged.

### 2026-05-02: Release creation is now gated by a pre-check

The "Error handling" section describes GitHub Release creation as unconditional. This is no longer accurate: [ADR-055](055-end-to-end-idempotent-publish-recovery.md) introduces a `CodeForgeClient::find_release_by_tag` pre-check so that an existing published release is treated as success and skipped, while an existing draft release produces an actionable error requiring manual intervention. The "no roll-back of registry publish on GitHub failure" guarantee is itself unchanged.
