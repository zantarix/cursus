...
### 2026-04-06: "No new configuration" principle and silent-exclusion rule have an exception

This ADR's "No new configuration" principle and its rule that private packages are silently excluded from the entire publish workflow are no longer absolute. [ADR-043](043-publish-private-packages-to-github-releases.md) introduces `[git].publish_private_packages`, a Cursus-specific configuration that opts listed private packages into receiving git tags and GitHub Releases (without registry publication) during `cursus publish`. Packages not listed in `publish_private_packages` continue to be silently skipped per this ADR.
