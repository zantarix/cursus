...
pub async fn run(cli: cli::Cli, env: Env) -> anyhow::Result<ExitCode> {
	...
	match cli.command {
		...
		command => {
			let config = model::config::load(&env).await?;
			match command {
				Some(cli::Command::Change(args)) => {
					cli::cmd_change(&args, &cli.global, config).await
				}
				Some(cli::Command::Prepare(args)) => cli::cmd_prepare(&args, dry_run, config).await,
				Some(cli::Command::Publish(args)) => cli::cmd_publish(&args, dry_run, config).await,
				Some(cli::Command::Ci(args)) => cli::cmd_ci(&args, dry_run, config).await,
				None => cli::cmd_change(&cli::ChangeArgs::default(), &cli.global, config).await,
