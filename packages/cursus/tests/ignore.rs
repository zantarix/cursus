...
async fn run_change_with_ignore(
	members: &[(&str, &str)],
	...
	common::run_cursus(args, dir.path()).await
}

#[tokio::test]
async fn ignored_package_is_excluded_from_change_targets() {
...
#[tokio::test]
async fn ignored_package_cannot_be_targeted_by_change() {
...
#[tokio::test]
async fn glob_pattern_excludes_multiple_packages() {
...
#[tokio::test]
async fn ignoring_all_packages_gives_informative_error() {
	// When every project is filtered out, the error must mention [global].ignore
	// rather than the generic "No projects found" message.
	let result = run_change_with_ignore(&[("app", "0.1.0")], vec!["app".to_string()], None).await;
	assert!(
		result.is_err(),
