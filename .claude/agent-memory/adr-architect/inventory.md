...
- **ADR-005** (Proposed): GitHub Releases. Errata: ADR-011 is authoritative reference for `build_command` execution semantics.
...
- **ADR-011** (Proposed): Command Execution Strategy — standardize shell execution via `/bin/sh -c` for all user-configurable commands, migrating `lock_command` from whitespace splitting. Covers dry-run, error handling, and working directory conventions.
